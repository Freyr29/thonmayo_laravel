...
class PanierController extends Controller
{
    ...
    public function index()
    {
        ...
        if(!empty($panier)) {
            $all_sandwich = Sandwich::select('id_sandwich', 'nom_sandwich', 'prix')->get();
            $all_snacks = Snacks::select('id_snack', 'nom_snack', 'prix')->get();
            $all_menus = Menus::select('id_menu', 'nom_menu', 'prix')->get();
            $all_boissons = Boissons::select('id_boisson', 'nom_boisson', 'prix')->get();
            foreach($all_sandwich as $sandwich) {
                array_push($data["sandwich"], array($sandwich["id_sandwich"], $sandwich["nom_sandwich"], $sandwich["prix"]));
            }
            foreach($all_snacks as $snack) {
                array_push($data["snacks"], array($snack["id_snack"], $snack["nom_snack"], $snack["prix"]));
            }
            foreach($all_menus as $menu) {
                array_push($data["menus"], array($menu["id_menu"], $menu["nom_menu"], $menu["prix"]));
            }
            foreach($all_boissons as $boisson) {
                array_push($data["boissons"], array($boisson["id_boisson"], $boisson["nom_boisson"], $boisson["prix"]));
            }
